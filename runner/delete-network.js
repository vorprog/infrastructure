...
const routeTable = exec(`aws ec2 describe-route-tables --filter 'Name=tag:Name,Values=infrastructure' --query 'RouteTables[0]'`);
...
const natGateway = exec(`aws ec2 describe-nat-gateways --filter 'Name=tag:Name,Values=infrastructure' --query 'NatGateways[0]'`);
exec(`aws ec2 delete-nat-gateway --nat-gateway-id ${natGateway.NatGatewayId}`);

const elasticIp = exec(`aws ec2 describe-addresses --filter 'Name=tag:Name,Values=infrastructure' --query 'Addresses[0]'`)
exec(`aws ec2 release-address --allocation-id ${elasticIp.AllocationId}`);

const tgwVpcAttachment = exec(`aws ec2 describe-transit-gateway-vpc-attachments --filter 'Name=tag:Name,Values=infrastructure' --query 'TransitGatewaysVpcAttachments[0]'`)
...
const subnet = exec(`aws ec2 describe-subnets --filter 'Name=tag:Name,Values=infrastructure' --query 'Subnets[0]'`);
exec(`aws ec2 delete-subnet --subnet-id ${subnet.SubnetId}`);

const vpc = exec(`aws ec2 describe-vpcs --filter 'Name=tag:Name,Values=infrastructure' --query 'Vpcs[0]'`);
