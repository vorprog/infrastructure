...
--tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=infrastructure}]'
--query 'Subnet'`);
...
--tag-specifications 'ResourceType=transit-gateway-attachment,Tags=[{Key=Name,Value=infrastructure}]'`);

const elasticIp = exec(`aws ec2 allocate-address
--domain vpc
--tag-specifications 'ResourceType=elastic-ip,Tags=[{Key=Name,Value=infrastructure}]'
--query 'Address'`);

const natGateway = exec(`aws ec2 create-nat-gateway 
--allocation-id ${elasticIp.AllocationId}
--subnet-id ${subnet.SubnetId}
--tag-specifications 'ResourceType=natgateway,Tags=[{Key=Name,Value=infrastructure}]'
--query 'NatGateway'`);
